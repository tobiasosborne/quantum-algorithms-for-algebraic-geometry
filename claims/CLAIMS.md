...
Current criterion: PRD D25 (2026-09-07) supersedes D22's stricter originality
requirement. Historical D22 novelty refutations retain their original scope;
they do not by themselves disqualify a non-QFT/Grover/DQI mechanism under D25.
C-374 onward records the current useful generator-selection results and their
explicit quantum-source access, rather than an explicit-coefficient speedup.
...
<!-- R11 statements below enter as SKETCH pending final integration review. -->

### C-365 (C-R11-HARMONIC-BATH-CONSERVATION)
- statement: For D-R11-HERMITIAN-COMPLEX, every trace-preserving Kraus family
  in the unital star algebra generated by d,d† satisfies E*(P)=P. Every
  Lindblad generator with Hermitian Hamiltonian and jumps in that same algebra
  satisfies L*(P)=0. Thus harmonic population Tr(P rho) is conserved at every
  step/time. Additional jumps outside this algebra are not covered.
- status: SKETCH
- depends-on: D-R11-HERMITIAN-COMPLEX
- where-proved: scouting/root-frontiers-r11.md section 2; commutation and
  Heisenberg-generator calculation.
- where-tested: checkers/explore/cp_geometry_r11.py; finite complex-unitary
  fixtures and harmonic-projector mutation, within its stated ranges.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.4;
  final integration review pending.
- north-star relevance: a reachability obstruction for a specified bath
  algebra, not an impossibility theorem for all cohomology preparation.

### C-366 (C-R11-DUAL-NUMBER-FUSION)
- statement: For D-R11-DUAL-NUMBER-FUSION, ||mu||=sqrt(2), the flagged
  instrument is trace preserving, and success on two identical normalized pure
  inputs with x=|a|² is 2x-3x²/2<=2/3, with equality at x=2/3. In the stated
  positive metric, mu†mu(e tensor e)=0 but
  (mu tensor 1)(1 tensor mu†)(e tensor e)=e tensor e.
- status: SKETCH
- depends-on: D-R11-DUAL-NUMBER-FUSION
- where-proved: scouting/root-frontiers-r11.md section 3; explicit matrices
  and maximization on 0<=x<=1.
- where-tested: checkers/explore/cp_geometry_r11.py, fusion-normalization
  mutation and finite probability checks.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.4;
  final integration review pending.
- north-star relevance: a normalized nonreduced-scheme operation and a metric
  counterexample; no growing-family advantage is asserted.

### C-367 (C-R11-BORN-METRIC-CANCELLATION)
- statement: For D-R11-METRIC-SAMPLER, the raw outcome-weighted ensemble is
  sum_i p_i |v_i><v_i|=H^(1/2) C_s H^(1/2)/(c Tr H), with
  p_i=w_i(s_i†H s_i)/(c Tr H). The displayed reweighting accepts with total
  probability a sum_i w_i/(c Tr H). These identities concern this instrument
  and do not rule out other balanced-metric algorithms.
- status: SKETCH
- depends-on: D-R11-METRIC-SAMPLER
- where-proved: scouting/root-frontiers-r11.md section 4; exact cancellation
  of the conditional-vector denominator by the Born weight.
- where-tested: checkers/explore/cp_geometry_r11.py, complex positive metrics
  and born-weight mutation.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.4;
  final integration review pending.
- north-star relevance: refutes the displayed free-normalization implementation
  of a proposed surface-metric update, not its entire computational problem.

### C-368 (C-R11-CHANNEL-PERIPHERAL-SEMISIMPLICITY)
- statement: Every D-R11-FINITE-CHANNEL has only size-one Jordan blocks at
  eigenvalues of modulus one. Interior eigenvalues may have nontrivial Jordan
  blocks; no efficient classical simulation or absence of useful transients
  is implied.
- status: SKETCH
- depends-on: D-R11-FINITE-CHANNEL
- where-proved: scouting/root-frontiers-r11.md section 6; trace-norm power
  boundedness contradicts growth along a peripheral Jordan chain.
- where-tested: checkers/explore/cp_geometry_r11.py illustrates allowed
  finite nilpotent transients in absorbing channels; it does not numerically
  prove the general spectral assertion.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.4;
  final integration review pending.
- north-star relevance: excludes an unphysical spectral amplification proposal
  while preserving interior transient and noncommutative alternatives.

### C-369 (C-R11-THERMAL-SYZYGY-GRAM)
- statement: For D-THERMAL-SYZYGY-EMITTER, the unnormalized successful bath
  state is binom(N+n,N-d) F_d†F_d/(M_N alpha²). Its algebraic kernel is the
  tuple-syzygy space ker F_d and its trace is the emission probability.
  C3 conjugation is essential to this orientation. Syzygies are zero-weight
  directions; the identity is not a procedure for sampling them from the bath.
- status: SKETCH
- depends-on: D-THERMAL-SYZYGY-EMITTER
- where-proved: scouting/thermal-cp-frontiers-r11.md section 3.1; irreducible
  unitary covariance, polarized Fock trace and the binomial occupation sum.
- where-tested: explicit two-generator control in the same memo section 3.3;
  uniform proof is algebraic, not inferred from a finite numerical run.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.1;
  final integration review pending.
- north-star relevance: an exact environment-state interpretation of a
  Macaulay Gram matrix. The mechanism is eligible under D25; this identity
  alone establishes no same-access quantum advantage.

### C-370 (C-R11-SYZYGY-ABSORPTION)
- statement: D-THERMAL-SYZYGY-ABSORPTION is CPTP and its live probability is
  q(t)=Tr exp(-t B†B)/L. With s_d=dim ker F_d and the stated positive gap eta,
  0<=q(t)-s_d/L<=(1-s_d/L)exp(-eta t). For 0<epsilon,delta<1, taking
  t>=eta^(-1)log(4/epsilon), per-trial probability bias at most epsilon/4,
  and O(epsilon^(-2)log(1/delta)) independent trials estimates s_d/L to
  additive epsilon with failure at most delta. Generator implementation,
  initial preparation and total repeated evolution time are additional costs.
- status: SKETCH
- depends-on: D-THERMAL-SYZYGY-ABSORPTION
- where-proved: scouting/thermal-cp-frontiers-r11.md section 3.2; complete
  absorbing Lindbladian, spectral decomposition and bounded Bernoulli sampling.
- where-tested: algebraic control family in the same memo section 3.3.
- referee: verdicts/thermal-cp-frontiers-r11.md, independent derivation 1.1;
  final integration review pending.
- north-star relevance: a conditional heat-trace nullity estimator; it does
  not supply its own geometric compiler or prove an advantage. QSVT/heat-kernel
  processing is eligible under the revised D25 criterion.

### C-371 (C-R11-MONOMIAL-FIRST-LOSS)
- statement: For D-R11-MONOMIAL-FIRST-LOSS, with c_t=#{u:lambda_u>=t},
  Pr(tau=t)=c_t/D_A and Pr(tau>k)=rank(S^k)/D_A
  =sum_u max(lambda_u-k,0)/D_A. The identical law is obtained by sampling
  one uniform standard monomial and classically following its successor until
  it vanishes. For a general contraction T and a maximally mixed input, the
  corresponding first-loss probability is
  (||T^(t-1)||_F²-||T^t||_F²)/D_A, which is generally metric-dependent and
  not a similarity-invariant Jordan statistic.
- status: SKETCH
- depends-on: D-R11-MONOMIAL-FIRST-LOSS
- where-proved: scouting/geometry-frontiers-r11.md section 4; orthogonal
  strings and the complete survival/defect instrument.
- where-tested: bounded diagnostics in that memo section 7; they do not
  establish a lower bound for every possible profile-recovery algorithm.
- referee: verdicts/geometry-frontiers-r11.md section 2; scoped identities
  accepted, final canonical integration review pending.
- north-star relevance: exact scheme-thickness statistics with a matched
  classical sampler, not a quantum advantage.

### C-372 (C-R11-CUBIC-SCHEME-SCATTERING)
- statement: For D-R11-CE-FOCK-SUPERCHARGE, Q_g²=0 and its weight-three
  Hermitian restriction has successive couplings (g,1,g). In
  D-R11-CE-SCATTERING-OUTPUT its endpoint amplitude equals
  i[cos(t/2)sin(Omega t/2)/Omega-sin(t/2)cos(Omega t/2)],
  Omega=sqrt(1+4g²). At g=sqrt(3)/2,t=pi the amplitude is i; for g tending
  to zero at fixed middle coupling, a fixed positive transition probability
  requires time Omega(g^(-2)). D-R11-CE-CUBIC-SCHEME is C[X]/(X³) for g>0.
  The resonant chain is exactly spin-3/2 J_x and its common scalar has a
  fixed-size classical formula.
- status: SKETCH
- depends-on: D-R11-CE-CUBIC-SCHEME, D-R11-CE-FOCK-SUPERCHARGE,
  D-R11-CE-SCATTERING-OUTPUT
- where-proved: scouting/ce-scattering-r11.md sections 2--4; exact occupation
  rules, elimination and two reflection-sector exponentials.
- where-tested: checkers/explore/cp_geometry_r11.py, weighted sectors zero
  through eight, complex amplitudes and two CE mutations.
- referee: verdicts/geometry-frontiers-r11.md section 5 explicitly approves
  the scoped identities; final canonical integration review pending.
- north-star relevance: physical higher-obstruction diagnostic; exact known
  perfect-state-transfer reduction is historically explicit and eligible under
  D25, but this fixed-size model has no asymptotic computational advantage.

### C-373 (C-R11-NONREDUCED-SYZYGY-BASELINE)
- statement: For D-R11-NONREDUCED-SYZYGY-CONTROL, F_d F_d† is diagonal
  with entries s_gamma, its norm squared is d(d-1), its smallest positive
  normalized Gram eigenvalue is 2/[d(d-1)], and dim ker F_d=M_(d-4) with
  L=2M_(d-2). At N=d the emitter success is 4/[(n+1)(n+2)]. The specified
  classical bright/dark sampler has exactly the same ideal live-bit law q(t)
  as the absorbing experiment; scalar-evaluation precision is charged in a
  finite implementation. If d/n tends to c>0, the nullity fraction tends to
  c²/[2(c+1)²]. No coefficient-list advantage follows on this family.
- status: SKETCH
- depends-on: D-R11-NONREDUCED-SYZYGY-CONTROL, C-369, C-370
- where-proved: scouting/thermal-cp-frontiers-r11.md section 3.3; disjoint
  Macaulay row fibres, complete-intersection syzygy and occupation averages.
- where-tested: independent algebraic review of the exact row weights,
  normalization, kernel dimension and classical sampler in the verdict.
- referee: verdicts/thermal-cp-frontiers-r11.md, explicit root control review;
  final integration review pending.
- north-star relevance: a nonreduced family with large matrices, a polynomial
  normalized gap and a resolvable signal whose output is still classically easy.

### C-374 (C-R12-SYZYGY-GRAM-SOURCE-AND-PAIR-LAW)
- statement: For D-SYZYGY-PAIR-TUPLE, the tuple generates the affine
  nonreduced length-four ideal (z0²,z1²), and its constant-coefficient syzygies
  form K=ker C. D-SYZYGY-PAIR-SOURCE emits the unnormalized bath state C†C/3
  with probability 2/3, yielding rho_C=C†C/2 after success. Every rank-two
  flat support is realized. The normalized pair law is
  |det C[:,J]|²=det(P_U[J,J]), equivalently a squared complementary Pluecker
  coordinate of K. Its positive-weight pairs are precisely the original
  generating pairs. No coefficient list or controllable emitter is supplied.
- status: PROVED
- depends-on: D-SYZYGY-PAIR-TUPLE, D-SYZYGY-PAIR-SOURCE,
  D-SYZYGY-PAIR-OUTPUT, D-THERMAL-SYZYGY-EMITTER
- where-proved: argument/syzygy-generator-selection.md section 1;
  scouting/syzygy-plucker-r12.md sections 2--3.
- where-tested: checkers/explore/syzygy_plucker_r12.py; complex coisometry,
  Fock source, complementary-minor and conjugation-mutation checks.
- referee: verdicts/syzygy-advantage-r12.md, independent source/geometry
  derivation and explicit approval of the canonical statement.
- north-star relevance: relates physical source data to useful reduction of
  a based redundant generator tuple; the bare fixed ideal itself is known.

### C-375 (C-R12-EIGHT-COPY-GENERATOR-SELECTION)
- statement: D-SYZYGY-GENERATOR-PROTOCOL solves D-SYZYGY-GENERATOR-OUTPUT
  with success at least 175/256, at most eight delivered original copies,
  at most 4w Fredkin gates and eight Hadamards, and 2w+1 live qubits,
  w=ceil(log2 q), before separately charged source and output costs.
  For q=2d,d>=3, every D-SYZYGY-GENERATOR-COMPARATOR succeeding with
  probability at least 2/3 on every promised tuple obeys
  T(T-1)>=d(d-2)/[3(d-1)], hence T=Omega(sqrt q). Strict cap-eight
  separation holds for every even q>=340. The lower bound is unconditional
  in the stated fixed-cap, independent-source, adaptive separate-copy model;
  it does not apply to coefficient lists, public hidden partitions, controlled
  emitters, cross-copy quantum memory or expected stopping costs. The quantum
  protocol uses no QFT, Grover, DQI or amplitude amplification.
- status: PROVED
- depends-on: C-374, C-362, D-SYZYGY-GENERATOR-OUTPUT,
  D-SYZYGY-GENERATOR-COMPARATOR, D-SYZYGY-GENERATOR-PROTOCOL
- where-proved: argument/syzygy-generator-selection.md sections 2--5;
  hidden balanced-partition extension of C-362's common transcript minorization.
- where-tested: checkers/explore/syzygy_plucker_r12.py; swap and HOM matrix
  laws, exact partition combinatorics, threshold and public-partition mutation.
  Finite checks do not replace the uniform transcript proof.
- referee: verdicts/syzygy-advantage-r12.md, independent ANY-pair derivation,
  explicit PROVED-registration approval and six-gate D25 assessment.
- north-star relevance: a useful original-generator selection problem with
  an unconditional quantum COPY advantage and an eligible D25 mechanism.
  A two-photon diagnostic tests the instrument; startup/preparation costs may
  dominate total runtime and no practical coefficient-list advantage is claimed.

### C-376 (C-R12-TWENTY-COPY-GENERATOR-VOLUME-SAMPLER)
- statement: D-SYZYGY-PAIR-SWAP-PROTOCOL uses at most twenty delivered
  copies and returns an always-output pair distribution within TV
  (3/4)^10<1/16 of D-SYZYGY-PAIR-OUTPUT for every promised C. Each accepted
  two-copy trial succeeds with probability 1/4 and gives the exact exterior
  ray wedge² U, whose sorted-coordinate law is the target. Every comparator
  in D-SYZYGY-PAIR-CLASSICAL-COMPARATOR with TV error at most 1/16 requires
  T(T-1)>=3q/16 for even q. This is a stronger prescribed output law than
  C-375's any-generating-pair task and has its own fixed copy cap.
- status: PROVED
- depends-on: C-374, C-362, D-SYZYGY-PAIR-OUTPUT,
  D-SYZYGY-PAIR-CLASSICAL-COMPARATOR, D-SYZYGY-PAIR-SWAP-PROTOCOL
- where-proved: scouting/syzygy-plucker-r12.md sections 3--5; exact
  antisymmetric instrument, independent capped repetition and C-362 transfer.
- where-tested: checkers/explore/syzygy_plucker_r12.py, source/minor/swap/HOM
  identities and six deliberately failing mutations.
- referee: verdicts/syzygy-advantage-r12.md, independent DPP-source and
  twenty-copy theorem approval with original-copy/output scope retained.
- north-star relevance: an additional volume-weighted basis guarantee using
  only ordinary swap interference, eligible under D25 in its source-data model.

### C-377 (C-R13-FAT-POINT-BASIS-SOURCE)
- statement: For D-R13-SYZYGY-BASIS-SOURCE, the based tuple generates
  (z0,z1)^(r-1), whose affine quotient has length r(r-1)/2. Its constant
  syzygies are ker C. A set of r original labels generates the same ideal
  exactly when its coefficient minor is nonzero. The degree-(r-1) Fock
  emitter is the isometry C†, delivering C†C/r with success one. No source
  control, coefficient list or hidden-partition metadata is supplied.
- status: PROVED
- depends-on: D-R13-SYZYGY-BASIS-SOURCE, D-R13-SYZYGY-BASIS-OUTPUT
- where-proved: scouting/syzygy-basis-r13.md section 1; homogeneous degree
  comparison, monomial count and exact Fock adjoint.
- where-tested: checkers/explore/syzygy_basis_r13.py; finite coisometry,
  source and permutation fixtures.
- referee: verdicts/syzygy-basis-r13.md, final PASS and explicit canonical
  PROVED approval after source/geometry review.
- north-star relevance: a useful reduction of an unknown based presentation
  of a nonreduced fat point, with an explicit fixed quantum-data source.

### C-378 (C-R13-GENERATOR-BASIS-COPY-LOWER)
- statement: For q=rd,r>=3,d>=2, every D-R13-SYZYGY-BASIS-COMPARATOR
  with uniform success at least 2/3 satisfies
  T(T-1)>=d(2/3-a)/(1-a)>=4q/(9r),
  a=r!d^r/[q(q-1)...(q-r+1)]. The hidden balanced partition and Haar
  vectors remain fixed throughout the transcript and are not public input.
  Consequently T=Omega(sqrt(q/r)) in this fixed-cap source-only model.
- status: PROVED
- depends-on: C-377, C-362, D-R13-SYZYGY-BASIS-COMPARATOR
- where-proved: scouting/syzygy-basis-r13.md section 2; r-group homogeneous
  Haar/Gaussian minorization and partition-independent reference transcript.
- where-tested: finite transversal counts in checkers/explore/syzygy_basis_r13.py;
  the uniform lower bound is analytic and is not inferred from those checks.
- referee: verdicts/syzygy-basis-r13.md, independent derivation and explicit
  PROVED approval of the exact and uniform lower bounds.
- north-star relevance: covers all adaptive within-copy measurements and
  unrestricted classical computation, not merely a particular classical sampler.

### C-379 (C-R13-QSVT-CONTENT-LUEDERS-BANK)
- statement: D-R13-HERMITIAN-CONTENT-ENCODING gives Hermitian projected-unitary
  encodings of A_k and each shifted Y_t. The signed eigenvalue-QSVT window
  reflection has degree O(k log(1/xi)) and a binary instrument approximation
  with diamond error at most 2xi under the stated full synthesis allocation.
  For the exact D-R13-CONTENT-WINDOW-BANK capped at L trials, outcome c has
  CP map [1-(1-1/H_k)^L] P_kc rho P_kc, and terminal failure trace is
  (1-1/H_k)^L independent of rho. All trials reuse the same data; time,
  ancillas, resets, padding and approximation errors are charged.
- status: PROVED
- depends-on: D-R13-HERMITIAN-CONTENT-ENCODING, D-R13-CONTENT-WINDOW-BANK,
  D-R13-BASIS-COMPILER-BUDGET
- where-proved: scouting/syzygy-basis-r13.md sections 4--6; explicit Hermitian
  SELECT/LCU, bounded sign polynomials, clean-space leakage and exact retry maps.
- where-tested: checkers/explore/syzygy_basis_r13.py; finite padded blocks,
  shifted encodings and full Lueders instruments. This does not numerically
  establish general phase-synthesis complexity.
- referee: verdicts/syzygy-basis-r13.md, Q1 repaired and independently closed;
  no outstanding objections to the signed QSVT or full-instrument analysis.
- north-star relevance: an explicit QSVT replacement for Fourier content
  measurements, eligible under D25.

### C-380 (C-R13-QSVT-GENERATOR-BASIS-ADVANTAGE)
- statement: With D-R13-BASIS-COMPILER-BUDGET and exact independent source
  copies, the QSVT/steering/certificate algorithm accepts each batch with
  probability at least 103/256>3/8. Every accepted column is exactly the
  exterior ray wedge^r U, including after the charged ancilla approximation.
  Correct-column certificate failures preserve that column, permitting the
  stated ten retries without new source copies. B_batch fresh batches and
  a fixed fallback solve D-R13-SYZYGY-BASIS-OUTPUT with failure at most delta,
  using O(r² log(1/delta)) delivered copies and polynomial r,log q,log(1/delta)
  compiler/processing cost. At delta=1/3 the cap is 6r². A sufficient strict
  copy separation from C-378 is q>(9r/4)(6r²)(6r²-1). This condition is a
  copy comparison; full processing is polynomial in log q when r is bounded
  by a fixed power of log q. Startup and delivery costs remain separately
  charged. No QFT, Grover, DQI or source-preparation inverse is used.
- status: PROVED
- depends-on: C-377, C-378, C-379, D-R13-BASIS-COMPILER-BUDGET,
  D-R13-SIGNED-CERTIFICATE
- where-proved: scouting/syzygy-basis-r13.md sections 3--10; tableau steering,
  full window-bank error, exact support invariant and retryable certificate.
- where-tested: checkers/explore/syzygy_basis_r13.py; certificate branches,
  finite bank maps and revised probability/cap constants.
- referee: verdicts/syzygy-basis-r13.md, final strengthened PASS with explicit
  approval of same-column retries, 103/256, three batches and 6r² copies.
- north-star relevance: the growing-rank D25 hit for useful original-generator
  selection in a fixed quantum-source model; not a coefficient-list or
  one-shot device-construction runtime advantage.

### C-381 (C-R13-DIRECT-ANTISYMMETRY-FALLBACK)
- statement: For D-R13-DIRECT-CERTIFICATE-FALLBACK, direct r-copy flat-source
  antisymmetry has probability 1/r^r, and the padded certificate accepts with
  probability gamma_r²/r^r>1/(4r^r). Independent capped repetition uses
  O(r^(r+1)log(1/delta)) copies and produces exact accepted exterior rays.
  This gives a separate non-Fourier upper that is polynomial in log q when
  r log r=O(log log q), with the same source and classical output as C-378.
  There is no r! numerator in the overlap and no claim of optimal rank cost.
- status: PROVED
- depends-on: C-377, C-378, D-R13-SIGNED-CERTIFICATE,
  D-R13-DIRECT-CERTIFICATE-FALLBACK
- where-proved: scouting/syzygy-basis-r13.md section 9; rank-one exterior
  support inside the r^r-dimensional flat source tensor power.
- where-tested: checkers/explore/syzygy_basis_r13.py; explicit r=3 overlap
  and antisymmetry-normalization mutation.
